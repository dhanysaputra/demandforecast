...
def plot_history_and_forecast(
    ...
    plt.plot(
        inv_df.index, inv_df["sales_observed"], label="Sales (Observed)", linewidth=2
    )
    ...
    plt.plot(
        forecast_series.index, forecast_series.values, label="Forecast", linewidth=2
    )
    ...
    if sim_df is not None:
        plt.plot(
            sim_df.index,
            sim_df["inv_end"],
            label="Inventory (Simulated)",
            linestyle="--",
            linewidth=2,
        )
        plt.plot(
            sim_df.index,
            sim_df["recommended_order"],
